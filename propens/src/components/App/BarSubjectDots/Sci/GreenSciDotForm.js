...
const GreenSciDotForm = props => {
  ...
  const changeHandler = event => {
    setScience({ ...science, [event.target.name]: event.target.value });
    console.log(event.target);
  };
  const submitForm = event => {
    event.preventDefault();
    // post the entry 
    setScience({ assignmentName: "", date: "", details: ""});
    props.closeNav();
  }
  return (
...
export default GreenSciDotForm;