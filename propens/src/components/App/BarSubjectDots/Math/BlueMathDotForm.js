...
const BlueMathDotForm = props => {
  ...
  const changeHandler = event => {
    setMath({ ...math, [event.target.name]: event.target.value });
    console.log(event.target);
  };
  const submitForm = event => {
    event.preventDefault();
    // post the entry 
    setMath({ assignmentName: "", date: "", details: ""});
    props.closeNav();
  }
  return (
    <StyledForm onSubmit={submitForm}>
        <h1>Let's Talk About Math!</h1>
...
export default BlueMathDotForm;